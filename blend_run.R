...
# Define the Blend Crossover
blend_crossover <- function(parent1, parent2) {
  blend_rate <- 0.8
  offspring1 <- parent1 + blend_rate * (parent2 - parent1)
  offspring2 <- parent2 + blend_rate * (parent1 - parent2)
...
runGA <- function(noRuns = 30, problem = "feature", popSize = 15, pcrossover = 0.8, pmutation = 0.1) {
  ...
  if (problem == "feature") {
    ...
    type = "binary"
    crossover = blend_crossover  # Use Arithmetic Crossover
    data <- getData()
