...
arithmetic_crossover <- function(parent1, parent2) {
  # Define a fixed crossover point (e.g., halfway through the chromosome)
  crossover_point <- length(parent1) / 2
  
  # Perform arithmetic crossover
  offspring1 <- c(parent1[1:crossover_point], parent2[(crossover_point + 1):length(parent2)])
  offspring2 <- c(parent2[1:crossover_point], parent1[(crossover_point + 1):length(parent1)])
...
runGA <- function(noRuns = 30, problem = "feature", popSize = 15, pcrossover = 0.5, pmutation = 0.1) {
  ...
  if (problem == "feature") {
    ...
    type = "binary"
    crossover = arithmetic_crossover  # Use Arithmetic Crossover
    data <- getData()
...
# Run with default population size (15), pcrossover = 0.5, pmutation = 0.1
#results1 <- runGA(noRuns = 30, problem = "feature")
# Run with a smaller population size (50) 
#results2 <- runGA(noRuns = 30, problem = "feature", popSize = 50, pcrossover = 0.7, pmutation = 0.2)
